...
| `tarkett_homogeneous_vinyl_colors.json` | Vinil (2) | Tarkett (3) | `collections[]` with `colors[]`, `documents`, `detailsSections`, `collection_image_url`, `url`, `characteristics` scraped from the official Tarkett homogeneous vinyl catalog |
...
| `tarkett_wood_collection_index.json` | Laminat (1), Parket (3) | Tarkett (3) | `parket[]` + `laminat[]` with official collection `description`, `shortDescription`, `keyFeatures`, `documents`, `heroImage`, `specs`, `url` scraped from the official Tarkett wood catalog |
