...
| `alpod_floor_collections.json` | Vinil (2), Parket (3), Deking (5) | Podovi (14, display brand) | `collections[]` with nested `colors[]`, generated by `tools/extract_alpod_floor_collections.js` from the public Alpod WooCommerce Store API for Parket/Vinil/Spoljašnje podne obloge plus the Parket menu pages `Essence Premium` and `Four Seasons`. Alpod is the upstream source/external URL, not the visible brand; product cards/PDP brand logo use the internal `Podovi` fallback logo because manufacturer logos are not available. Current payload: 851 items grouped into 13 collections (4 Vinil, 7 Parket, 2 Deking). Deking accessories, including `Završne lajsne`, remain mapped to category `5`, not category `11`. |
| `esd_colors.json` | Elektroprovodni (8) | Gerflor (6), Tarkett (3) | `collections[]` with `colors[]`; Tarkett iQ Granit SD uses explicit collection/color `brandId`, `price`/`priceUnit`, official self-hosted cover, color images, technical characteristics and shared collection PDFs. Tarkett ESD headers reuse the Tarkett nested collection builder; exact color slugs resolve before legacy suffix/prefix aliases. `getEsdColorProducts()` reuses the nested mapper for the homepage color count and `/api/home-colors`, merged with repository results before existing deduplication. |
...
- Homepage (`app/page.tsx` + `components/HomeProductTabs.tsx`) je sada katalog-first surface, ne hero/merchandising landing: SSR bira collection-first grupe po kategoriji, parsira početni filter query preko `lib/catalog/home-filter-url.ts`, a klijentski `HomeProductTabs` radi faceted filter i sinhronizuje ga sa History API-jem. Ponovljivi parametri `category`, `brand`, `type`, `application`, `collection` i `facet`, uz raspon debljine, `tab` i `sort`, jesu kanonski homepage deep-link contract; reload i Back/Forward moraju vratiti stanje, a nepovezani query parametri ostati sačuvani.
...
- Homepage filter brojači koriste `skip-own-group` semantiku: svaka opcija se meri preko svih ostalih aktivnih grupa, ali ne i preko svoje grupe. To važi za kategorije, tipove, primenu, brendove, kolekcije i deklarativne facete; neizabrana nula-opcija je disabled, a izabrana se nikad ne zaključava. Duge liste idu kroz deljeni „Prikaži još/manje" obrazac, izabrane vrednosti ostaju vidljive, a kolekcije imaju lokalnu pretragu.
- Na root ruti `Header` ispod `lg` renderuje `GlobalSearch` u `inline` varijanti i desnim filter dugmetom emituje `OPEN_HOME_FILTERS_EVENT` iz `lib/catalog/home-filter-events.ts`; na ostalim rutama isto mesto ostaje klasični navigacioni hamburger. Inline pretraga mora zadržati combobox/listbox/option contract i poništavati zastarele zahteve, tako da brže kucanje ne može vratiti stariji rezultat. Mobilni homepage intro (`Sve kolekcije`, count i opis) je namerno skriven, dok desktop uvod ostaje.
- Mobilna filter fioka mora ostati jedan deljeni state/panel sa desktop aside-om i zadržati dialog contract: scroll lock, Escape, pravi Tab/Shift+Tab focus trap, povratak fokusa, zatvaranje pri desktop resize-u, `100dvh`/safe-area i najmanje 44px touch mete. Sort `featured` se korisniku prikazuje kao „Preporučeno" (ne „Najnovije"), a `price` opcija se prikazuje samo kada aktivni skup ima validne cene. Wizard nije deo homepage filter data flow-a.
