...
11. **IZVORI: ASORTIMAN vs MATERIJAL** (pravilo vlasnika, 08.07.2026) — asortiman diktira DOBAVLJAČ (npr. Admonter kupujemo od Alpoda → nudimo SAMO dekore koje Alpod nudi, ništa ne dodajemo iz fabričkog kataloga), ali MATERIJAL za prikaz (fotografije, opisi, dokumentacija, specifikacije) vučemo sa ZVANIČNOG SAJTA PROIZVOĐAČA jer je opširniji i kvalitetniji. Dakle: lista proizvoda = dobavljač, sadržaj po proizvodu = proizvođač. **Slike kolekcija = proizvođačke „References" fotografije** (profesionalni enterijeri sa referentnih projekata; vlasnik ih tretira kao javno dostupan marketinški materijal — preuzimamo slobodno, ali UVEK migrirati u naš Supabase, nikad hotlink; preskakati fajlove sa imenom fotografa/copyright u nazivu).
...
- PDP (`app/proizvodi/[slug]/page.tsx`) eksplicitno koristi `fetchCache = 'force-no-store'`, jer je live provera otkrila da je Next Data Cache zadržao stare Supabase cene i posle novog deploy-a; nakon čišćenja cache-a svih 10 kolekcija je provereno u browseru. Filter konfiguracija i data flow nisu menjani: asortiman, specifikacije i vrednosti faceta ostali su isti; sortiranje po ceni koristi ažurirane vrednosti kroz postojeći DB-first resolver.
...
- **Svi brojači su živi**: kategorija, tip, primena, brend, kolekcija i dinamičke facet grupe računaju broj preko svih ostalih aktivnih uslova uz `skip-own-group` pravilo — opcije iste grupe se ne samosasecaju. Neizabrana opcija sa 0 rezultata je disabled/posivljena, dok izabrana ostaje dostupna za uklanjanje.
- **Duge liste više nisu odsečene**: deljeni `ExpandableFilterOptions` inicijalno prikazuje kompaktan broj opcija, zadržava izabrane vrednosti vidljivim i nudi „Prikaži još" / „Prikaži manje"; kolekcije dodatno imaju lokalnu pretragu preko `SearchableFilterOptions`, a brendovi više nisu ograničeni na prvih šest.
- **Mobilni/tablet header početne**: ispod `lg` breakpointa `Header` između wordmarka i desnog dugmeta renderuje stvarno inline polje `GlobalSearch` (bez click-only lupe); desno dugme sa sliders ikonom emituje `OPEN_HOME_FILTERS_EVENT` i otvara homepage filter fioku. Combobox ima listbox/option/active-descendant semantiku, a abort + request-id zaštita sprečava da stariji odgovor pretrage pregazi noviji. Klasični hamburger meni ostaje na ostalim rutama. Uvod „Sve kolekcije" + broj + opis je sakriven ispod `md`, da katalog i filteri krenu odmah ispod headera.
- **Fioka je ojačana za telefon i tastaturu**: pravi Tab/Shift+Tab focus trap, Escape zatvaranje, povratak fokusa na prethodni element, automatsko zatvaranje pri prelasku na desktop, `100dvh`/safe-area raspored, najmanje 44px touch mete i odvojena numerička polja „Od/Do" za debljinu. CTA ne prikazuje lažni rezultat dok se boje učitavaju ili kada učitavanje padne.
- **Pošteno sortiranje**: kurirani početni red je nazvan „Preporučeno" umesto nepouzdanog „Najnovije"; sortiranje po nazivu ostaje dostupno, a „Cena" se nudi samo kada aktivni skup zaista ima proizvode sa cenom. Wizard „Pronađi svoj pod" namerno nije deo ovog unapređenja.
- Contract pokrivenost je dopunjena fajlovima `tests/contracts/home-filter-url-contract.test.ts`, `home-filter-utils-contract.test.ts` i `home-filter-ui-contract.test.ts`.
...
- **Novi brend `Admonter` (id '16')** u mock-data (union u brand-repository ga nosi i na produkciju); UVEDEN helper `isAlpodImportBrand()` u `lib/catalog/spec-normalize.ts` — svih 10 mesta koja su proveravala `brandId === '14'` za Alpod ponašanje sada koriste helper (prepare-colors, product-routes, brand-curation, product-repository, obe page.tsx). ⚠️ Ako se ubuduće izdvoji još neki proizvođački brend iz Alpod importa — samo proširiti helper.
- PDP popravke: `toSpecKey` unifikovan sa loaderom preko `specKeyFromLabel` (bug: 20/25 spec labela se prikazivalo duplo sa `?color=` — sada varijanta zamenjuje kolekcijski agregat, jedna tabela); galerija boje prikazuje SVE slike (svoč + room-shot; strelice/tačkice rade; overlay sloj preko pre-render steka); pre-render neaktivnih boja je `loading="lazy"` (ranije eager za sve — i 221 slika na jednom PDP-u); `FlooringCalculator` MONTIRAN na PDP kad postoji `coveragePerPackage` (ranije mrtva komponenta); merge za `?color=` daje ljudsko ime, celu galeriju (i za OG meta), čitljiv opis.
- Snapshot testovi (resolver, color-api) osveženi jer su hvatali stari dupli-spec bug (specCount 17→13 je nameravana promena); novi testovi: `admonter-enrichment-contract` (6), `pdp-color-merge-contract` (2). Svih 237 contract testova prolazi.
...
## 2026-09-08: BLOQ removed from the assortment

User requested full public removal of BLOQ. Removed 18 collections, 210 colors and 229 public image/logo assets, including direct color/PDP fallback sources. Do not re-import or restore BLOQ without a new user request. Keep shared Tekstilne ploče category `4`, Gerflor and Tarkett Desso. Retirement cleanup uses exact old identities because all 210 BLOQ color IDs lacked a brand prefix; prefix-only filtering misses favorites and old color URLs. Supabase was verified to contain zero BLOQ brands/products/colors.
