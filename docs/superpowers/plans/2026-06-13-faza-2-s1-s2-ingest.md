...
**Amandman posle vizuelne provere pilota (KRITIČNO — ispravka mehanizma):** spec §6.2 je pretpostavio da `documents`/`room_scene_images`/`collection_image_url` iz JSON-a stižu do PDP-a kroz `getVinylCollectionProducts()` loader. NETAČNO: Gerflor vinil kolekcije postoje kao redovi u Supabase `products` tabeli (seed iz `lib/data/mock-data.ts`), a resolver je DB-first (`resolve-product.ts` → `findBySlug` → DB red), pa loader nikad ne stigne za PDP. DB red se obogaćuje iz JSON kolekcije kroz `enrichProductFromCollectionData` (kopira opis+specs), ali NE kopira nova polja. Slike boja se i dalje prikazuju jer idu kroz zaseban `/api/colors` put koji čita JSON direktno. ISPRAVKA: `enrichProductFromCollectionData` (`lib/product-page/resolve-product.ts:25`) dopunjen da iz JSON kolekcije kopira i `documents` (kad ih DB proizvod nema), `collection_image_url` (postaje primarna slika), i `room_scene_images` (dodate u `images`). Generička, guard-ovana izmena (radi samo kad polja postoje na kolekciji). Task 5 (loader) ostaje za fallback put (kolekcije van DB-a). NE upisuje se ništa u Supabase bazu — JSON ostaje jedini izvor obogaćivanja.
