...
async function scrapeMissingFast() {
  console.log('🚀 Fast missing vinyl scraper...\n');
    ...
    for (let i = 0; i < collectionLinks.length; i++) {
      ...
      for (const colorUrl of colorLinks) {
        ...
        if (slug && !existingSlugs.has(slug)) {
          missingUrls.push(colorUrl);
        }
      }

      if (missingUrls.length > 0) {
        process.stdout.write(`${colorLinks.length} colors, ${missingUrls.length} missing\n`);
        
        // Scrape missing ones
        for (let j = 0; j < missingUrls.length; j++) {
          const colorUrl = missingUrls[j];
        ...
        process.stdout.write(`${colorLinks.length} colors, all exist\n`);
      }
    }
  }
...
scrapeMissingFast().catch(console.error);
