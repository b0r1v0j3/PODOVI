...
// Navigate to Taralay Impression Compact page and scrape all colors
async function scrapeTaralayImpressionCompactColors() {
  const url = 'https://www.gerflor-cee.com/products/taralay-impression-compact';
  ...
  try {
    console.log('Navigating to Taralay Impression Compact page...');
    ...
    const colors = await page.evaluate(() => {
      ...
      const selectors = [
        'a[href*="/products/taralay-impression-compact-"]',
        '[data-color-code]',
        '.color-item',
        '.product-color',
        '.product-card',
      ...
      for (const selector of selectors) {
        ...
        if (elements.length > 0) {
          elements.forEach(el => {
            ...
            if (href && href.includes('taralay-impression-compact-')) {
              const text = el.textContent?.trim() || '';
              const codeMatch = text.match(/(\d{4})/) || href.match(/taralay-impression-compact-(\d{4})/);
              ...
              if (code) {
                ...
                const nameMatch = text.match(/\d{4}\s+(.+)/i) || 
                                 text.match(/-([a-z-]+)/i) ||
                                 href.match(/taralay-impression-compact-\d{4}-(.+?)(?:-hd|\?|$)/i);
      ...
      // Alternative: look for all links with taralay-impression-compact
      if (extracted.length < 50) {
        const allLinks = Array.from(document.querySelectorAll('a[href*="taralay-impression-compact"]'));
        allLinks.forEach(link => {
          const href = link.href || link.getAttribute('href');
          const text = link.textContent?.trim() || '';
          const codeMatch = href.match(/taralay-impression-compact-(\d{4})/) || text.match(/(\d{4})/);
          const code = codeMatch ? codeMatch[1] : '';
    ...
    const colorsWithSlugs = colors.map(color => {
      ...
      return {
        ...color,
        slug: `taralay-impression-compact-${color.code}-${slugName}`,
        sku: null,
        collection_slug: 'taralay-impression-compact',
        collection_name: 'Taralay Impression Compact',
      };
    });
...
(async () => {
  try {
    const colors = await scrapeTaralayImpressionCompactColors();
    
    // Save to JSON file
    const outputPath = path.join(__dirname, 'taralay-impression-compact-colors.json');
    ...
    console.log(`Taralay Impression Compact: ${colors.length} colors`);
    console.log(`Difference: ${acousticColors.length - colors.length} colors`);
    
    // Find which colors are missing in Compact
    const acousticCodes = new Set(acousticColors.map(c => c.code));
    const compactCodes = new Set(colors.map(c => c.code));
    const missingInCompact = acousticColors.filter(c => !compactCodes.has(c.code));
    
    if (missingInCompact.length > 0) {
      console.log(`\nColors in Acoustic but not in Compact (${missingInCompact.length}):`);
      missingInCompact.forEach(c => console.log(`  ${c.code} ${c.name}`));
    }
    
    // Find which colors are in Compact but not in Acoustic
    const extraInCompact = colors.filter(c => !acousticCodes.has(c.code));
    if (extraInCompact.length > 0) {
      console.log(`\nColors in Compact but not in Acoustic (${extraInCompact.length}):`);
      extraInCompact.forEach(c => console.log(`  ${c.code} ${c.name}`));
    }
