...
const colors = JSON.parse(fs.readFileSync('tools/taralay-millenium-acoustic-colors.json', 'utf8'));
...
// Get all ZIP files in root directory, sorted by creation time (oldest first = 1 to 32)
...
const tempDir = path.join(__dirname, '..', 'tmp', 'taralay-millenium-acoustic-extract');
...
const targetDir = 'public/images/products/vinyl/taralay-millenium-acoustic-order';
...
for (let i = 0; i < Math.min(zipFiles.length, colors.length); i++) {
  ...
  try {
    ...
    processed.push({
      code: color.code,
      name: color.name,
      image: `/images/products/vinyl/taralay-millenium-acoustic-order/${targetFileName}`
    });
