...
function buildCollection(collectionCategory, categoryConfig, products, categoryMap) {
  ...
  return {
    ...
    shortDescription: `${rawName} - ${colors.length} ${categoryConfig.key === 'deking' ? 'artikala' : 'dekora'} u katalogu`,
...
function buildManualCollection(rawCollection) {
  ...
  return {
    ...
    shortDescription: `${rawCollection.name} - ${colors.length} opcija u katalogu`,
