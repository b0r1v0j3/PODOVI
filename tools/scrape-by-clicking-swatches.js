...
async function scrapeByClicking() {
  console.log('🚀 Scraping by clicking on swatches...\n');
  ...
  for (let i = 0; i < collections.length; i++) {
    ...
    // Click "View all" to show all swatches
    ...
    // Get all swatches and click each one
    const swatches = page.locator('[class*="color-item"], [class*="swatch"], button[aria-label*="color"]');
    const swatchCount = await swatches.count();
    console.log(`  Found ${swatchCount} swatches (očekujem ${collection.expected})`);

    const colorUrls = [];
    
    // Click each swatch to get URL
    for (let j = 0; j < Math.min(swatchCount, collection.expected + 5); j++) {
      try {
        const swatch = swatches.nth(j);
        await swatch.scrollIntoViewIfNeeded({ timeout: 5000 });
        await swatch.click({ timeout: 5000 });
        await page.waitForTimeout(500);
        
        const currentUrl = page.url();
        if (currentUrl.includes('/products/') && /\d{8,}$/.test(currentUrl)) {
          if (!colorUrls.includes(currentUrl)) {
            colorUrls.push(currentUrl);
          }
        }
        ...
        // Re-click "View all" if needed
        const viewAllBtn2 = page.locator('a:has-text("View all"), button:has-text("View all")').first();
        if (await viewAllBtn2.count() > 0 && await viewAllBtn2.isVisible().catch(() => false)) {
          await viewAllBtn2.click();
          await page.waitForTimeout(1000);
        }
      } catch (error) {
    ...
    console.log(`  Extracted ${colorUrls.length} unique URLs`);

    // Now scrape each color
    for (let j = 0; j < colorUrls.length; j++) {
      const colorUrl = colorUrls[j];
      const urlSlug = colorUrl.split('/products/')[1]?.replace(/-\d{8,}$/, '') || null;
      
      if (!urlSlug || existingSlugs.has(urlSlug)) {
        process.stdout.write(`  [${j + 1}/${colorUrls.length}] ⏭️  skip\n`);
        continue;
      }

      process.stdout.write(`  [${j + 1}/${colorUrls.length}] `);
...
scrapeByClicking().catch(console.error);
