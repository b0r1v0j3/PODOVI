...
const extractDir = path.join(rootDir, 'tmp', 'extracted-planet-colors');
const planetDir = path.join(rootDir, 'public', 'images', 'products', 'vinyl', 'mipolam-planet');
...
// Get Mipolam Planet colors
const planetCollection = colorsData.collections.find(c => c.slug === 'mipolam-planet');
if (!planetCollection) {
  console.error('Mipolam Planet collection not found!');
  process.exit(1);
}

console.log(`Found ${planetCollection.colors.length} colors for Mipolam Planet\n`);
...
console.log('\nOrganizing color images for Mipolam Planet...\n');
...
extractedDirs.forEach(dirName => {
  ...
  files.forEach(file => {
    ...
    if (fileName.includes('collection') || (fileName.includes('planet') && !fileName.match(/\d{4}/))) {
      return;
    }
    
    // Try to match file to a color
    planetCollection.colors.forEach(color => {
      ...
      if (fileName.includes(colorCode) || 
        ...
        const destPath = path.join(planetDir, colorFileName);
...
console.log(`\n✅ Organized ${organizedCount} color images for Mipolam Planet`);
console.log(`📁 Location: ${planetDir}`);
