...
const extractDir = path.join(rootDir, 'tmp', 'extracted-symbioz-colors');
...
// Get Mipolam Symbioz colors
const symbiozCollection = colorsData.collections.find(c => c.slug === 'mipolam-symbioz');
if (!symbiozCollection) {
  console.error('Mipolam Symbioz collection not found!');
  process.exit(1);
}

console.log(`Found ${symbiozCollection.colors.length} colors for Mipolam Symbioz\n`);
...
console.log('\nOrganizing color images for Mipolam Symbioz...\n');
...
extractedDirs.forEach(dirName => {
  ...
  files.forEach(file => {
    ...
    if (fileName.includes('collection') || (fileName.includes('symbioz') && !fileName.match(/\d{4}/))) {
      return;
    }
    
    // Try to match file to a color
    symbiozCollection.colors.forEach(color => {
...
console.log(`\n✅ Organized ${organizedCount} color images for Mipolam Symbioz`);
