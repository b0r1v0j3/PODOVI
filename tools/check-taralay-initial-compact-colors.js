...
// Navigate to Taralay Initial Compact page and scrape all colors
async function scrapeTaralayInitialCompactColors() {
  const url = 'https://www.gerflor-cee.com/products/taralay-initial-compact';
  ...
  try {
    console.log('Navigating to Taralay Initial Compact page...');
    ...
    const colors = await page.evaluate(() => {
      ...
      const selectors = [
        'a[href*="/products/taralay-initial-compact-"]',
        '[data-color-code]',
        '.color-item',
        '.product-color',
        '.product-card',
      ...
      for (const selector of selectors) {
        ...
        if (elements.length > 0) {
          elements.forEach(el => {
            ...
            if (href && href.includes('taralay-initial-compact-')) {
              const text = el.textContent?.trim() || '';
              const codeMatch = text.match(/(\d{4})/) || href.match(/taralay-initial-compact-(\d{4})/);
              ...
              if (code) {
                ...
                const nameMatch = text.match(/\d{4}\s+(.+)/i) || 
                                 text.match(/-([a-z-]+)/i) ||
                                 href.match(/taralay-initial-compact-\d{4}-(.+?)(?:-hd|\?|$)/i);
      ...
      // Alternative: look for all links with taralay-initial-compact
      if (extracted.length < 20) {
        const allLinks = Array.from(document.querySelectorAll('a[href*="taralay-initial-compact"]'));
        allLinks.forEach(link => {
          const href = link.href || link.getAttribute('href');
          const text = link.textContent?.trim() || '';
          const codeMatch = href.match(/taralay-initial-compact-(\d{4})/) || text.match(/(\d{4})/);
          const code = codeMatch ? codeMatch[1] : '';
...
(async () => {
  try {
    const colors = await scrapeTaralayInitialCompactColors();
    
    // Save to JSON file
    const outputPath = path.join(__dirname, 'taralay-initial-compact-colors.json');
    fs.writeFileSync(outputPath, JSON.stringify(colors, null, 2));
    console.log(`\nSaved ${colors.length} colors to ${outputPath}`);
    
    // Compare with Initial Acoustic colors
    if (fs.existsSync('tools/taralay-initial-acoustic-colors.json')) {
      const acousticColors = JSON.parse(fs.readFileSync('tools/taralay-initial-acoustic-colors.json', 'utf8'));
      console.log(`\n=== Comparison with Taralay Initial Acoustic ===`);
      console.log(`Taralay Initial Acoustic: ${acousticColors.length} colors`);
      console.log(`Taralay Initial Compact: ${colors.length} colors`);
      
      // Find which colors are in Initial Compact
      ...
      console.log(`\nColors in Initial Compact that match Acoustic: ${inCompact.length}/${colors.length}`);
      console.log(`Colors ONLY in Initial Compact (new): ${onlyInCompact.length}`);
      console.log(`Colors ONLY in Acoustic (not in Compact): ${onlyInAcoustic.length}`);
      
      if (onlyInCompact.length > 0) {
        console.log(`\n⚠ NEW colors in Initial Compact (${onlyInCompact.length}):`);
      ...
      if (inCompact.length === colors.length && onlyInCompact.length === 0 && onlyInAcoustic.length === 0) {
        console.log(`\n=== CONCLUSION ===`);
        console.log(`✓ All ${colors.length} colors in Taralay Initial Compact are the SAME as Taralay Initial Acoustic`);
        console.log(`✓ These collections use identical colors`);
      } else if (inCompact.length === colors.length && onlyInAcoustic.length > 0) {
        console.log(`\n=== CONCLUSION ===`);
        console.log(`✓ All colors in Initial Compact exist in Initial Acoustic`);
        console.log(`⚠ But Initial Acoustic has ${onlyInAcoustic.length} additional colors`);
        console.log(`⚠ Compact is a SUBSET of Acoustic`);
      } else {
        console.log(`\n=== CONCLUSION ===`);
        console.log(`⚠ Taralay Initial Compact has ${onlyInCompact.length} NEW colors that don't exist in Acoustic`);
        console.log(`⚠ These are DIFFERENT colors, not the same`);
      }
    } else {
      console.log('\n⚠ Taralay Initial Acoustic colors file not found for comparison');
    }
