...
async function scrapeWithNetwork() {
  console.log('🚀 Scraping with network monitoring...\n');
  ...
  const collections = [
    { url: 'https://www.gerflor-cee.com/products/premium-acoustic', slug: 'premium-acoustic', expected: 29 },
    { url: 'https://www.gerflor-cee.com/products/premium-compact', slug: 'premium-compact', expected: 81 },
    { url: 'https://www.gerflor-cee.com/products/taralay-millenium-acoustic-order', slug: 'taralay-millenium-acoustic-order', expected: 32 },
    { url: 'https://www.gerflor-cee.com/products/taralay-millenium-compact', slug: 'taralay-millenium-compact', expected: 32 }
  ];
  ...
  for (const collection of collections) {
    ...
    page.on('response', async (response) => {
      ...
      if (url.includes('/products/') && /\d{8,}$/.test(url)) {
    ...
    // Try to find color elements and hover/click them to trigger navigation
    const colorElements = page.locator('[class*="color"], [data-product], [class*="swatch"]');
    const colorCount = await colorElements.count();
    
    console.log(`  Found ${colorCount} color elements`);
    
    // Try to extract all possible product links from current page state
    const pageUrls = await page.evaluate(() => {
      const urls = [];
      
      // Check all link elements
      document.querySelectorAll('a').forEach(a => {
        if (a.href && a.href.includes('/products/') && /\d{8,}$/.test(a.href)) {
          urls.push(a.href);
        }
      });
      
      // Check data attributes
      document.querySelectorAll('[data-url], [data-product-url], [data-href]').forEach(el => {
        const dataUrl = el.getAttribute('data-url') || el.getAttribute('data-product-url') || el.getAttribute('data-href');
        if (dataUrl && dataUrl.includes('/products/')) {
          const fullUrl = dataUrl.startsWith('http') ? dataUrl : `https://www.gerflor-cee.com${dataUrl}`;
          urls.push(fullUrl);
        }
      });
    ...
    console.log(`  Found ${pageUrls.length} URLs in page`);
    capturedUrls.push(...pageUrls);
  }

  // Remove duplicates
  const uniqueUrls = [...new Set(capturedUrls)];
  console.log(`\n📊 Total unique URLs captured: ${uniqueUrls.length}`);
  ...
  console.log(`Missing from existing data: ${missingUrls.length}\n`);
...
scrapeWithNetwork().catch(console.error);
