...
const targetCollections = ['premium-acoustic', 'premium-compact', 'taralay-millenium-acoustic-order', 'taralay-millenium-compact'];

async function scrapeAllPages() {
  console.log('🚀 Scraping ALL pages to find missing collections...\n');
  ...
  try {
    ...
    while (hasMorePages && currentPage <= maxPages) {
      ...
      // Extract ALL colors from this page
      ...
      // Check if we found any target collections
      const foundTargets = pageColors.filter(c => targetCollections.includes(c.collection_slug));
      if (foundTargets.length > 0) {
        console.log(`  🎯 Found colors from target collections:`);
        const byCollection = {};
        foundTargets.forEach(c => {
          if (!byCollection[c.collection_slug]) {
            byCollection[c.collection_slug] = 0;
          }
          byCollection[c.collection_slug]++;
        });
        Object.entries(byCollection).forEach(([slug, count]) => {
    ...
    collections.forEach(col => {
      const isTarget = targetCollections.includes(col.slug);
      console.log(`  ${isTarget ? '🎯' : '  '} ${col.name}: ${col.colorCount} colors`);
    });
    ...
    console.log('🎯 Target collections status:\n');
