...
async function scrapeAllVinyl() {
  console.log('🚀 Starting complete vinyl scraping...\n');
  ...
  for (const config of configs) {
    ...
    while (clicks < 50) {
      ...
      if (isVisible && !isDisabled) {
        ...
        clicks++;
        if (clicks % 5 === 0) console.log(`   Clicked ${clicks} times...`);
      } else {
        break;
      }
    }
    ...
    for (let i = 0; i < collectionLinks.length; i++) {
      ...
      console.log(`\n2. [${i + 1}/${collectionLinks.length}] Processing: ${collectionSlug}`);
      ...
      for (let j = 0; j < colorLinks.length; j++) {
        ...
        try {
          ...
          process.stdout.write(`✓ ${colorData.code || 'N/A'}\n`);
        } catch (error) {
    ...
    console.log(`\n✅ ${config.type}: ${results[config.type].length} colors extracted`);
  }
  ...
  console.log(`\n✅ Complete! Saved ${outputData.metadata.total} colors to: ${outputPath}`);
  return outputData;
}
...
async function extractColorData(page, type, collectionSlug) {
  ...
  // Normalize characteristics - extract key values
  ...
  // Normalize characteristics object - Dimenzije first, Ukupna debljina second
  const normalizedCharacteristics = {};
  
  // Add Dimenzije first if exists
  if (dimension) {
    normalizedCharacteristics['Dimenzije'] = dimension;
  }
  
  // Add Ukupna debljina second if exists
  if (overallThickness) {
    normalizedCharacteristics['Ukupna debljina'] = overallThickness;
  }

  // Add other characteristics (excluding duplicates)
  Object.entries(characteristics).forEach(([key, value]) => {
    // Skip already added keys and dimension-related duplicates
...
scrapeAllVinyl()
  .then((result) => {
    console.log(`\n📊 FINAL SUMMARY:`);
    console.log(`   Total colors: ${result.metadata.total}`);
    console.log(`   Homogeneous: ${result.metadata.homogeneous}`);
    console.log(`   Heterogeneous: ${result.metadata.heterogeneous}`);
  })
  .catch((error) => {
