...
async function scrapeMissingFromColorsTab() {
  console.log('🚀 Scraping missing 174 colors from Colors tab...\n');
  ...
  console.log(`Existing: ${existingData.total}`);
  console.log(`Target: 908`);
  console.log(`Missing: ${908 - existingData.total}\n`);

  const newColors = [];

  // Go to HETEROGENEOUS category page
  console.log('=== HETEROGENEOUS (Colors tab) ===');
  ...
  if (await colorsTab.count() > 0) {
    console.log('Clicking Colors tab...');
    await colorsTab.click();
    await page.waitForTimeout(2000);
  }

  // Click "Show more" many times to load ALL colors
  console.log('Loading all colors...');
  for (let i = 0; i < 100; i++) {
    const btn = page.locator('button:has-text("Show more")').first();
    if (await btn.count() === 0) {
      console.log(`  Button not found, stopping at ${i} clicks`);
      break;
    }
    ...
    if (isDisabled !== null) {
      console.log(`  Button disabled after ${i} clicks`);
      break;
    }
    
    if (!isVisible) {
      console.log(`  Button not visible after ${i} clicks`);
      break;
    }
    ...
    try {
      ...
      if ((i + 1) % 10 === 0) console.log(`  Clicked ${i + 1} times...`);
    } catch (error) {
      console.log(`  Click failed at ${i} clicks: ${error.message}`);
      break;
    }
  }
  
  console.log('Finished clicking Show more');

  // Get ALL color links from Colors tab
  ...
  console.log(`\nFound ${allColorLinks.length} color links on Colors tab`);
  ...
  // Extract data for missing colors
  for (let i = 0; i < missingLinks.length; i++) {
    ...
    try {
      ...
      if (cdnImgUrl) {
        const imageDir = path.join('public/images/products/vinyl', collectionSlug, slug);
        ...
        if (fs.existsSync(imgPath)) {
          localImagePath = `/images/products/vinyl/${collectionSlug}/${slug}/${fileName}`;
        }
      }
...
scrapeMissingFromColorsTab().catch(console.error);
