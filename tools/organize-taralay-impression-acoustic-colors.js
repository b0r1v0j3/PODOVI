...
// Read Taralay Impression Acoustic colors from JSON
...
if (!collection) {
  console.error('Taralay Impression Acoustic collection not found in JSON');
  process.exit(1);
}

console.log(`Found ${collection.colors.length} colors for Taralay Impression Acoustic`);
...
if (zipFiles.length !== collection.colors.length) {
  console.warn(`Warning: Found ${zipFiles.length} ZIP files but expected ${collection.colors.length} colors`);
}

// Create temp directory for extraction
const tempDir = path.join('.', 'temp-taralay-impression-acoustic');
...
for (let i = 0; i < Math.min(zipFiles.length, collection.colors.length); i++) {
  ...
  try {
    ...
    color.image = `/images/products/vinyl/taralay-impression-acoustic/${colorSlug}.jpg`;
...
console.log(`\n\n=== Summary ===`);
console.log(`Processed: ${processedCount}/${collection.colors.length} colors`);
if (errors.length > 0) {
