...
const extractDir = path.join(rootDir, 'tmp', 'extracted-affinity-colors');
const affinityDir = path.join(rootDir, 'public', 'images', 'products', 'vinyl', 'mipolam-affinity');
...
// Get Mipolam Affinity colors
const affinityCollection = colorsData.collections.find(c => c.slug === 'mipolam-affinity');
if (!affinityCollection) {
  console.error('Mipolam Affinity collection not found!');
  process.exit(1);
}

console.log(`Found ${affinityCollection.colors.length} colors for Mipolam Affinity\n`);
...
console.log('\nOrganizing color images for Mipolam Affinity...\n');
...
extractedDirs.forEach(dirName => {
  ...
  files.forEach(file => {
    ...
    if (fileName.includes('collection') || fileName.includes('affinity') && !fileName.match(/\d{4}/)) {
      return;
    }
    
    // Try to match file to a color
    affinityCollection.colors.forEach(color => {
      ...
      if (fileName.includes(colorCode) || 
        ...
        const destPath = path.join(affinityDir, colorFileName);
...
console.log(`\n✅ Organized ${organizedCount} color images for Mipolam Affinity`);
console.log(`📁 Location: ${affinityDir}`);
