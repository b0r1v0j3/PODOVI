...
-- =============================================
-- Cenovnik — unos cena dobavljača (na pregled/odobravanje)
-- Ništa odavde NE ide automatski u produkciju.
-- =============================================
CREATE TABLE IF NOT EXISTS price_submissions (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  status TEXT NOT NULL DEFAULT 'new'
    CHECK (status IN ('new','reviewed','approved','rejected','archived')),
  payload JSONB NOT NULL DEFAULT '{}'::jsonb,
  note TEXT,
  tags JSONB NOT NULL DEFAULT '[]'::jsonb,
  file_name TEXT,
  file_mime TEXT,
  submitted_by TEXT NOT NULL DEFAULT 'tatjana',
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
...
CREATE INDEX IF NOT EXISTS idx_price_submissions_created_at ON price_submissions(created_at DESC);
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname = 'public' AND tablename = 'price_submissions' AND policyname = 'Service manage price_submissions'
  ) THEN
    CREATE POLICY "Service manage price_submissions"
